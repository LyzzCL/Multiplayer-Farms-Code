...
{
  /// <summary>
  /// Draws the mailbox notification for farmhands living on additional farms.
  /// </summary>
  internal static class MailboxOverlay
  {
    public static void OnRenderedWorld(object? sender, RenderedWorldEventArgs e)
    {
      if (!Context.IsWorldReady || Game1.IsMasterGame) return;
      if (Game1.player.mailbox.Count <= 0) return;

      // Only draw where cabin mailboxes can exist.
      if (Game1.currentLocation is not Farm && !FarmRegistry.IsCustomFarmLocation(Game1.currentLocation?.Name))
        return;
    ...
    private static bool HasMatchingCabin(GameLocation location, string homeLocationName)
    {
      ...
      {
        if (building.isCabin && building.HasIndoorsName(homeLocationName)) return true;
      }
    ...
    private static void DrawMailboxNotification(SpriteBatch spriteBatch, Point mailboxPosition)
    {
      ...
      spriteBatch.Draw(Game1.mouseCursors,
        Game1.GlobalToLocal(Game1.viewport,
          new Vector2(mailboxPosition.X * 64, mailboxPosition.Y * 64 - 96 - 48 + bob)), new Rectangle(141, 465, 20, 24),
        Color.White * 0.75f, 0f, Vector2.Zero, 4f, SpriteEffects.None, layerDepth + 0.000001f);
      
      spriteBatch.Draw(Game1.mouseCursors,
        Game1.GlobalToLocal(Game1.viewport,
          new Vector2(mailboxPosition.X * 64 + 32 + 4, mailboxPosition.Y * 64 - 64 - 24 - 8 + bob)),
        new Rectangle(189, 423, 15, 13), Color.White, 0f, new Vector2(7f, 6f), 4f, SpriteEffects.None,
        layerDepth + 0.00001f);
    }
  }
}
